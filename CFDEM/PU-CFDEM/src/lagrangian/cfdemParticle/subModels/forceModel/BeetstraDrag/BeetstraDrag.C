    ...
    different_diameter_(false)
{
    ...
    if (propsDict_.found("different_diameter")) 
    {
        different_diameter_=true;
        Info << "Using different particle diameter for drag." << endl;
    }
...
void BeetstraDrag::setForce() const
{
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
		...
		if(different_diameter_)
		{
			dPrim_ = readScalar(propsDict_.lookup("dd"));
			Vs = dPrim_*dPrim_*dPrim_*M_PI/6;
		}
		else 
                {
			dPrim_ = ds/scale_;
		...
		nuf = nufField[cellI];
                ...
                /*
		if( verbose_ && index < 2) //&& index>100 && index < 105)
                {
		...
		*/
                if(verbose_ && index <1)
                {
                    ...
                    Info << "Rep = " << Rep << endl;
                    Info << "Volp = " << Vs << endl;
                    Info << "alps = " << localPhiP << endl;
                    //Info << "CD = " << CD << endl;
		    Info << "betaP = " << beta << endl;		   		    
                    Info << "drag = " << drag << endl;
                }		
