...
defineTypeNameAndDebug(WenYuParcelDrag, 0);
...
WenYuParcelDrag::WenYuParcelDrag
(
...
void WenYuParcelDrag::setForce() const
{
    // get viscosity field
    #ifdef comp
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
		...
		// Parcel approach: drag based on original particle diameter
		// ds: parcel diameter from DEM
		// scale it by a factor of npart_^(-1/3) to obtain the primary particle diameter
		//std::cout << "parcel diameter is "<<ds<<std::endl;
		ds = ds*pow(npart_, -1.0/3.0);
		//std::cout << "npart_ is "<<npart_<<std::endl;
		//std::cout << "primary particle diameter is "<<ds<<std::endl;
		...
		nuf = nufField[cellI];
		    ...
		    //Info << "Using WenYuParcelDrag ..."<<endl;
		    drag *= npart_;
