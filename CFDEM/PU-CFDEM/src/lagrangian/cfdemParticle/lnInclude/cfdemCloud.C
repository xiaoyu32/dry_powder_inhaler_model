...
:
    ...
    positions_(NULL),
    velocities_(NULL),
//_AO- Particle angular momentum & type
    omegas_(NULL),
    types_(NULL),
//_AO- Particle angular momentum & type  
    fluidVel_(NULL),
...
//_AO- Particle angular momentum & type
    ...
    dataExchangeM().destroy(types_,1);
//_AO- Particle angular momentum & type   
...
//_AO- Particle angular momentum & type    
    ...
    dataExchangeM().getData("type","scalar-atom",types_);
//_AO- Particle angular momentum & type
    
    
    //for( int i = 0; i < numberOfParticles(); ++i )
    //    std::cout<<"Particle type: "<<types_[i][0]<<std::endl;
    
    // -- fetch additional DEM data for the force models if required --
    for (int i=0;i<cfdemCloud::nrForceModels();i++) cfdemCloud::forceM(i).getDEMdata();
...
void Foam::cfdemCloud::findCells()
{
        Info<< "Entered findCells"<<endl;
        Info<<"positions= "<<positions_<<"      cellIDs= "<<cellIDs_<<"         numberOfparticles= "<<numberOfParticles()<<endl;
    locateM().findCell(NULL,positions_,cellIDs_,numberOfParticles());
        Info<< "Left findCells"<<endl;
...
//_AO
void Foam::cfdemCloud::setVel(double**& vel)
{
    for(int index = 0;index <  numberOfParticles(); ++index)
    {
        for(int i=0;i<3;i++){
...
//_AO
void Foam::cfdemCloud::setOmega(double**& ome)
{
    for(int index = 0;index <  numberOfParticles(); ++index)
    {
        for(int i=0;i<3;i++){
            omegas_[index][i] = ome[index][i];
        }
    }
}


void Foam::cfdemCloud::setType(double** & typ)
{
    for(int index =0; index < numberOfParticles(); ++index)
    {
    	types_[index][0] = typ[index][0];
    }	
}

void Foam::cfdemCloud::setRadii(double**& rad)
{
    for(int index = 0;index <  numberOfParticles(); ++index)
    {
        radii_[index][0] = rad[index][0];
    }
}

void Foam::cfdemCloud::setCellID(double**& cellI)
{
    for(int index = 0;index <  numberOfParticles(); ++index)
    {
        cellIDs_[index][0] = cellI[index][0];
    }
}
...
vector Foam::cfdemCloud::omega(int index)
{
    vector ome;
...
label Foam::cfdemCloud::type(int index)
{
    //label ty = static_cast<label>(types()[index][0]);
    return (label)types()[index][0]; 	
}
...
const forceModel& Foam::cfdemCloud::forceM(int i) const
{
    return forceModel_[i];
}

int Foam::cfdemCloud::nrForceModels() const
{
...
)
{
    ...
    if(!ignore())
    {
	    ...
	    /*** moved the giveDEMdata() from here to the commValues method (JK) ***/ 
...
//_AO- Particle angular velocity
	...
	dataExchangeM().allocateArray(types_,0.,1);
//_AO- Particle angular velocity
	...
	for (int i=0;i<cfdemCloud::nrForceModels();i++) cfdemCloud::forceM(i).reAllocArrays();
...
//_AO_Post processing allocation memory
bool Foam::cfdemCloud::reAllocArraysPost() const
{
        // get arrays of new length
        dataExchangeM().allocateArray(positions_,0.,3);
        dataExchangeM().allocateArray(velocities_,0.,3);
...
void cfdemCloud::calcDdtVoidfraction(volScalarField& voidfraction) const
{
	...
	Info<< " fvc::ddt(voidfraction) * scale; scale = "<<scale<<endl;
...
void Foam::cfdemCloud::commValues()
{
    for (int i=0;i<nrForceModels();i++)
    {
        forceM(i).commValues();
    ...
    /* added this from evolve (JK) */
    if(!ignore())
    {
            
        if( dataExchangeM().coupleEnd() )
        {
	    Info <<"Sending data to DEM..." << endl;
            // write DEM data
            if(verbose_) Info << " -giveDEMdata()" << endl;
            clockM().start(23,"giveDEMdata");
            giveDEMdata();
