...
#include "apiParcelVoidFraction.H"
...
defineTypeNameAndDebug(apiParcelVoidFraction, 0);

addToRunTimeSelectionTable
(
    voidFractionModel,
    apiParcelVoidFraction,
    dictionary
);
...
apiParcelVoidFraction::apiParcelVoidFraction
(
...
:
    ...
    nParcel_
    ( 
  	propsDict_.lookupOrDefault("nParcel", 1.0)
    ),
    apiType_
    (
    ...
    )
{
    ...
    Info<<"Coarsening Ratio: "<<nParcel_<<endl;
    ...
    if (propsDict_.found("interpolation")){
        interpolation_=true;
        Warning << "interpolation for apiParcelVoidFraction does not yet work correctly!" << endl;
...
void apiParcelVoidFraction::setvoidFraction(double** const& mask,double**& voidfractions,double**& particleWeights,double**& particleVolumes) const
{
    reAllocArrays();
    ...
    for(int index=0; index< particleCloud_.numberOfParticles(); index++)
    {
	    ...
	    const scalar pvolume = 4./3.*radius*radius*radius*3.1415*weight();
	    
            scalar volume = ( ptype == apiType_ ) ? pvolume * nParcel_ : pvolume;
