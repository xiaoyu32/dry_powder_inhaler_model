...
#include "dividedVoidFractionPrev.H"
...
defineTypeNameAndDebug(dividedVoidFractionPrev, 0);

addToRunTimeSelectionTable
(
    voidFractionModel,
    dividedVoidFractionPrev,
    dictionary
);
...
dividedVoidFractionPrev::dividedVoidFractionPrev
(
    ...
    interpolation_(false)
{
    ...
    if (propsDict_.found("interpolation")){
        interpolation_=true;
        Warning << "interpolation for dividedVoidFractionPrev does not yet work correctly!" << endl;
...
void dividedVoidFractionPrev::setvoidFraction(double** const& mask,double**& voidfractions,double**& particleWeights,double**& particleVolumes) const
{
    reAllocArrays();
    ...
    for(int index=0; index< particleCloud_.numberOfParticles(); index++)
    {
        ...
        //{
            ...
            if (cellID >= 0)  // particel centre is in domain
            {
                ...
                scalar newAlpha = voidfractionPrev_[cellID]- volume*centreWeight/cellVol;
                if(newAlpha > alphaMin_) voidfractionPrev_[cellID] = newAlpha;
                else
                {
                    voidfractionPrev_[cellID] = alphaMin_;
...
//    interpolationCellPoint<scalar> voidfractionInterpolator_(voidfractionPrev_);
            ...
            for(int subcell=0;subcell<cellsPerParticle_[index][0];subcell++)
            {
                ...
                if(cellID >= 0)
                {
                    if(voidfractionAtPos > 0)
                        ...
                        voidfractions[index][subcell] = voidfractionPrev_[cellID];
                } 
                else
                {
