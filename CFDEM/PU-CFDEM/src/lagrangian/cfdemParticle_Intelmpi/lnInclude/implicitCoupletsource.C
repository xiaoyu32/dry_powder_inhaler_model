...
#include "implicitCoupletsource.H"
...
defineTypeNameAndDebug(implicitCoupletsource, 0);

addToRunTimeSelectionTable
(
    momCoupleModel,
    implicitCoupletsource,
    dictionary
);
...
implicitCoupletsource::implicitCoupletsource
(
    ...
    )
{
    ...
    Info << "tsource uses KslNext instead of KslPrev" << endl;
}
...
tmp<volScalarField> implicitCoupletsource::impMomSource() const
{
    ...
    if(1-tsf < 1e-4) //tsf==1
    {
        ...
        tsource() = KslNext_; //KslPrev_; CMB March 25 2016-- trying to solve issue with restarted simulations having jumps in slip velocity 
    }else
    {
...
void Foam::implicitCoupletsource::resetMomSourceField() const
{
