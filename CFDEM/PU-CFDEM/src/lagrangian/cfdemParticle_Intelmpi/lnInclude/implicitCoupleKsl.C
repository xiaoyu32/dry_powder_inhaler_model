...
defineTypeNameAndDebug(implicitCoupleKsl, 0);
...
implicitCoupleKsl::implicitCoupleKsl
(
...
tmp<volScalarField> implicitCoupleKsl::impMomSource() const
{
    ...
    if(1-tsf < 1e-4) //tsf==1
    {
	    ...
	    Ur = U_[cellI] - Us_[cellI];
            ...
            if( mag(Ur) > SMALL && alpha_[cellI] < maxAlpha_) //momentum exchange switched off if alpha too big
	    {
                //KslNext_[cellI] = mag(particleCloud_.forceM(0).impParticleForces()[cellI])
                //            / Ur
                //            / particleCloud_.mesh().V()[cellI];
                KslNext_[cellI] = ( particleCloud_.forceM(0).impParticleForces()[cellI] & Ur )
				 / ( Ur & Ur )
                            	 / particleCloud_.mesh().V()[cellI];
            }
...
void Foam::implicitCoupleKsl::resetMomSourceField() const
{
