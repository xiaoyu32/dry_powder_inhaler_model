...
namespace Foam
{
	...
	{	
		...
		double Y_star(0); double G_star(0); double R_star(0); double m_star(0);
        	...
        	e_n = coefficientRestitution; 	
		mu_f = coefficientFriction;
		...
		if(collisionModel==0)
		{
			...
			Y_star = 1. / ( (1-pow(poissonsRatio,2))/youngsModulus + (1-pow(poissonsRatio,2))/youngsModulus );
			G_star = 1. / ( 2*(2+poissonsRatio)*(1-poissonsRatio)/youngsModulus + 2*(2+poissonsRatio)*(1-poissonsRatio)/youngsModulus );
		...
		{					
			Info << " " << endl;
			Info << " During collision relative distance = " << delta_n << " between particle " << j << " and " << i << endl;
			//for (int dir=0;dir<3;dir++)
			...
			if (liquid_transfer)
			{
				...
				if ( liquidVol[i][j] > 0 )
				{	
					Info << " Volume of liquid bridge = " << liquidVol[i][j] << endl;
		...
		{ 
			if(liquid_transfer) //|| ( particles.liquidOn(i) > 0 ) || ( particles.liquidOn(j) > 0 ) )
			{
				if(liquidVol[i][j] > 0 )
				{
					Info << " During breaking up the bridge relative distance= " << delta_n << " between particle " << i << "\t and " << j << endl;
					for (int dir=0;dir<3;dir++)
						Info << " position of particle= " << i << " " << particles.position(i)[dir] << "\t and " << j << " " << particles.position(j)[dir] << endl;

					Info << " During breaking up volume of liquid bridge = " << liquidVol[i][j] << endl;
					...
					{
						Info << " Test delta_n > dist_rup " << endl;
		...
		*/
		...
		sigma_coll.xx() += 0.5 * dist[0] * ( F_n[0] + F_t[0] ); // fcoll[j][0]; 																										
		sigma_coll.yy() += 0.5 * dist[1] * ( F_n[1] + F_t[1] ); // fcoll[j][1]; 													
		sigma_coll.zz() += 0.5 * dist[2] * ( F_n[2] + F_t[2] ); // fcoll[j][2];				
		sigma_coll.xy() += 0.5 * dist[0] * ( F_n[1] + F_t[1] ); // fcoll[j][1]; 													
		sigma_coll.xz() += 0.5 * dist[0] * ( F_n[2] + F_t[2] ); // fcoll[j][2]; 
		sigma_coll.yz() += 0.5 * dist[1] * ( F_n[2] + F_t[2] ); // fcoll[j][2];
