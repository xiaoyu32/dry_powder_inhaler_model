	...
	{	
		...
		{					
			if(verbose)
			{
				...
				//for (int dir=0;dir<3;dir++)
					//Pout << " Vel_" << dir << " of particle = " << i << " " << particles.position(i)[dir] << "\t and " << j << " " << particles.position(j)[dir] << endl;
			}
			...
			{	
				...
				if(verbose)
				{
					...
					Pout << " v_j[" << dir << "]= " << setw(IOstream::defaultPrecision() + 6) << particles.velocity(j)[dir] << " " 
					     << " v_i[" << dir << "]= " << setw(IOstream::defaultPrecision() + 6) << particles.velocity(i)[dir];
					Pout << " x_j[" << dir << "]= " << setw(IOstream::defaultPrecision() + 6) << particles.position(j)[dir] << " " 
		...
		{ 
			if(liquid_transfer) //|| ( particles.liquidOn(i) > 0 ) || ( particles.liquidOn(j) > 0 ) )
			{
				if(liquidVol[i][j] > 0 )
				{
					...
					for (int dir=0;dir<3;dir++)
					{
						if(verbose) Pout << " position of particle= " << i << " " << particles.position(i)[dir] << "\t and " << j << " " << particles.position(j)[dir] << endl;
					}
