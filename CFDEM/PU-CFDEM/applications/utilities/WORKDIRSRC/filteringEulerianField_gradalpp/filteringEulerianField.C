...
namespace Foam
{
	...
	(
		...
		Field <Field < Field <scalar> > >& 	giiCondGradalppFilter_,
		Field < Field <scalar> >&  		numbergiiCondGradalppFilter_,
	...
	{			
		...
		// Binning Gradalpp
		volVectorField dalppdi = fvc::grad(1-baralpf_);		
		volScalarField dalppdx = sqrt( dalppdi.component(vector::X) * dalppdi.component(vector::X) );
		volScalarField dalppdy = sqrt( dalppdi.component(vector::Y) * dalppdi.component(vector::Y) );
		volScalarField dalppdz = sqrt( dalppdi.component(vector::Z) * dalppdi.component(vector::Z) );
		
		minGradalpp_ = Foam::min( dalppdx + dalppdy + dalppdz ).value();
		maxGradalpp_ = Foam::max( dalppdx + dalppdy + dalppdz ).value();	
		
		const scalar binGradalpp_ = (maxGradalpp_-minGradalpp_)/nBin_;
		...
		interpolationCellPoint<vector> dalppdiInterpolator_(dalppdi);
	    	...
	    	for(int index = 0; index <  maxIndex; index++)
		{
			...
			if (cellI > -1) // particle Found
			{
				...
				*/
				...
				GradalppInt = dalppdiInterpolator_.interpolate(position,cellI);  //* pow(mesh.C()[cellI],1./3.) ;	
				label binGradAlppI = floor( ( mag(GradalppInt) - minGradalpp_ ) /binGradalpp_ );
				
				// Cell values or at particle positions
				//label binGradAlppI = floor( ( (dalppdx[cellI]+dalppdy[cellI]+dalppdz[cellI]) - minGradalpp_ ) /binGradalpp_ );
								
                                if ( binGradAlppI < nBin_ && binGradAlppI > -1 ) // Avoid to sample out of the defined binning interval
				{
					for(int j=0;j<3;j++) 
					{	
					         giiCondGradalppFilter_[binGradAlppI][j][filterwidth] += gii_[index][j];
					}			      					      					      
					      numbergiiCondGradalppFilter_[binGradAlppI][filterwidth] += 1;				
				}				
	...
	(
		...
		bool GradientModel_,
		const scalar& minGradalpp_,
		const scalar& maxGradalpp_		 
	)		
	{
		...
		char charfilterwidthGradbaralpp[100]; 
		char charfilterwidthNGradbaralpp[100]; 		
		...
		for( int filterwidth = minfilterwidth; filterwidth <= maxfilterwidth; filterwidth += filterincrement )
		{
			...
			sprintf(charfilterwidthGradbaralpp,      "h_vs_Gradbaralpp_%dX%dX%d"			,Filter,Filter,Filter);		
			sprintf(charfilterwidthNGradbaralpp, 	 "Nrealization_Gradbaralpp_%dX%dX%d"		,Filter,Filter,Filter);
			...
			fileName outputfileGradbaralpp(charfilterwidthGradbaralpp);
			fileName outputfileNGradbaralpp(charfilterwidthNGradbaralpp);			
			...
			{								
				...
				sprintf(charfilterwidthGradbaralpp,	 "h_vs_Gradbaralpp_%dX%dX%d_npart=%d"			,Filter,Filter,Filter,nparticle_);
				sprintf(charfilterwidthNGradbaralpp,	 "Nrealization_Gradbaralpp_%dX%dX%d_npart=%d"		,Filter,Filter,Filter,nparticle_);
				...
				outputfileGradbaralpp = charfilterwidthGradbaralpp;
				outputfileNGradbaralpp = charfilterwidthNGradbaralpp;
			...
			else if (EulerEulerFiltering_)
			{
				...
				sprintf(charfilterwidthGradbaralpp,	 "Eulerian_h_vs_Gradbaralpp_%dX%dX%d_npart=%d"		,Filter,Filter,Filter,nparticle_);
				sprintf(charfilterwidthNGradbaralpp,	 "Eulerian_Nrealization_Gradbaralpp_%dX%dX%d_npart=%d"	,Filter,Filter,Filter,nparticle_);							
			...
			OFstream         str_giiGradbaralpp(mesh.time().path()/outputRelativePath_/runTime.timeName()/outputfileGradbaralpp);
			OFstream       str_nrealGradbaralpp(mesh.time().path()/outputRelativePath_/runTime.timeName()/outputfileNGradbaralpp);
			...
			Info << " Writing the file " << outputfile << ", " << outputfileVelSlip      << ", " << outputfileTaupVelSlip      << ", " << outputfileN 
								   ...
								   << ", " << outputfileGradbaralpp  << ", " << outputfileNGradbaralpp
			...
			str_giiGradbaralpp		<< "# Gradbaralpp \t (1-H_x) \t (1-H_y) \t (1-H_z) "   	<< nl;
			str_nrealGradbaralpp		<< "# Gradbaralpp \t Nreal			   "   	<< nl;
			
			for( int i = 0; i < nBin_; i++ )
			{
								
				str_nreal 	 	 << (i+1./2.)*(maxalpp_-minalpp_)/nBin_          << " " << numberCondalppFilter_[i][filterwidth]     << nl;
				str_nrealGradbaralpp 	 << (i+1./2.)*(maxGradalpp_-minGradalpp_)/nBin_  << " " << numbergiiCondGradalppFilter_[i][filterwidth] << nl;
								
				// Avoid to divide by zero
				if ( numberCondalppFilter_[i][filterwidth] == 0 ) numberCondalppFilter_[i][filterwidth] = 1; 
				if ( numbergiiCondGradalppFilter_[i][filterwidth] == 0 ) numbergiiCondGradalppFilter_[i][filterwidth] = 1; 
				...
				str_giiGradbaralpp << minGradalpp_+(i+1./2.)*(maxGradalpp_-minGradalpp_)/nBin_  
				          << " " << giiCondGradalppFilter_[i][0][filterwidth] / numbergiiCondGradalppFilter_[i][filterwidth] 
					  << " " << giiCondGradalppFilter_[i][1][filterwidth] / numbergiiCondGradalppFilter_[i][filterwidth] 
					  << " " << giiCondGradalppFilter_[i][2][filterwidth] / numbergiiCondGradalppFilter_[i][filterwidth] 	<< nl;	
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	//
	const scalar rhop(readScalar(propsDict.lookup("rhoParticle")));
	...
	scalar minGradalpp(0);
	scalar maxGradalpp(0);
	if(propsDict.found("GradientModel")) 
	{
		Info << "\nThe binning Gradalpp is active " << endl;
		GradientModel = true;	
		//minGradalpp=readScalar(propsDict.lookup("Min_Gradalpp"));
		//maxGradalpp=readScalar(propsDict.lookup("Max_Gradalpp"));
	}	
		...
		if ( runTime.timeName() != "0" )
		{
			...
			for(int FilterWidth = minFilterWidth; FilterWidth <= maxFilterWidth; FilterWidth += FilterIncrement )
			{
					...
					for ( int npartI = 0; npartI < npartParticleCoarsening.size(); npartI++ )
					{
						// Particle coarsening active or not
							...
							Foam::applyBins(parcelCloud,
									...
									NparcelgiiCondGradalppFilter[npartI],
									NparcelnumbergiiCondGradalppFilter[npartI],
									false,							 // Lagrangian filtering --> false, Eulerian filtering --> true
									EulerianVslipBin,
									GradientModel,minGradalpp,maxGradalpp				); 
				...
				(				
					...
					giiCondGradalppFilter,
					numbergiiCondGradalppFilter,	
					false,
					EulerianVslipBin,
					GradientModel,minGradalpp,maxGradalpp				
				);
				...
				if(EulerEulerFiltering)
				{
					...
					(				
						...
						Eulerian_giiCondGradalppFilter,					
						Eulerian_numbergiiCondGradalppFilter,					
						true,
