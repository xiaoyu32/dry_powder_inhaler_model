...
{ 
	...
	(
	...
	)
 	{
		...
		vectorField parForce(parList_.size(),vector(0,0,0));	
			
		// Parcel/Particle averaged drag force
		//vectorField parDragForce(parIDInSubVolume_.size(),vector(0,0,0));
		vectorField parDragForce(parList_.size(),vector(0,0,0));		
		...
		vectorField parFilteredRelativeVelocity(parList_.size(),vector(0,0,0));

		// Parcel/Particle averaged relative velocity
		//vectorField parRelativeVelocity(parIDInSubVolume_.size(),vector(0,0,0));	
		vectorField parRelativeVelocity(parList_.size(),vector(0,0,0));		
		...
		for(int parI=0; parI < parList_.size(); parI++)
		{
			dist = vector(0,0,0);
