...
namespace Foam
{
  namespace dragModels
  {
    ...
    {
      defineTypeNameAndDebug(DriftFluxRe,0);
      addToRunTimeSelectionTable(DriftFluxModel, DriftFluxRe, dictionary);
    }
  }
}
...
Foam::dragModels::driftFluxModels::DriftFluxRe::DriftFluxRe
(
...
:
   ...
   filterSize_
   (
       readScalar(dict.subDict("driftFluxModelProps").lookup("filterSize"))   
   ),
   g_
   (
       pair.g()
   ...
   Re_sys
   (
    readScalar(dict.subDict("driftFluxModelProps").lookup("Re_sys"))
    ),
...
void Foam::dragModels::driftFluxModels::DriftFluxRe::evolve()
{
  ...
  {   
    ...
    //if(( solidVolumeFraction[iCell] <= 0.05 ) || ( solidVolumeFraction[iCell] >= 0.45 ))
