...
#include "DriftFluxNNetLocalNewX12Y6NoWCNoCap.H"
...
namespace Foam
{
  namespace dragModels
  {
    ...
    {
      defineTypeNameAndDebug(DriftFluxNNetLocalNewX12Y6NoWCNoCap,0);
      addToRunTimeSelectionTable(DriftFluxModel, DriftFluxNNetLocalNewX12Y6NoWCNoCap, dictionary);
    }
  }
}
...
Foam::dragModels::driftFluxModels::DriftFluxNNetLocalNewX12Y6NoWCNoCap::DriftFluxNNetLocalNewX12Y6NoWCNoCap
(
...
:
   ...
   (
	...
	pair.dispersed().U().mesh(),
	dimensionedVector("DriftFluxDiffusion",dimensionSet(0,1,-2,0,0),Foam::vector(0,0,0))
   ),
   Xsgs
   (
	IOobject
   ...
   (
	...
	pair.dispersed().U().mesh(),
	dimensionedScalar( "gradYsgs",dimensionSet(0,0,-1,0,0), 0.0 )   
   ),
   sumXY
   (
	IOobject
   ...
   (
	...
	pair.dispersed().U().mesh(),
	dimensionedScalar("limiterVd",dimensionSet(0,0,0,0,0),0.0)
   ), 
   XnnModel_
   (
   ...
   */
   ...
   (
    ...
    equilibriumSolutionSw_
    (
       dict.subDict( "driftFluxModelProps" ).lookupOrDefault<Switch>("equilibriumSolution", false )
    ),
    Xmeans_(NULL),
    Xvars_(NULL),
   //XBmeans_(NULL),
...
void Foam::dragModels::driftFluxModels::DriftFluxNNetLocalNewX12Y6NoWCNoCap::evolve()
{
  ...
  diffusion = fvc::laplacian( driftFluxDiffusivity, driftFlux_ );
  ...
  */
  ...
  forAll( uSlip, iCell )
  {
     limiterVd[iCell ] = max( 
			       ...
			       ( 
     				    pow( 
				   	 0.8 + ( uSlip[iCell] & driftFlux_[iCell] )/
				   	 max( magSqr( uSlip[iCell] ), 1e-9 )
				   	 /solidVolumeFraction[iCell],
					 2.0
				       ), 
		  	            1e-9  
			       ), 
  ...
  dimensionedScalar tauinv
  (
     "tauinv",dimensionSet(0,0,-1,0,0), 0.01
  );
  
  Info<< "min( limiterVd ) == "<<min(limiterVd)<<endl
      << "max( limiterVd ) == "<<max(limiterVd)<<endl;
  
//with diffusion

  if( equilibriumSolutionSw_ )
  {
     Info<<"Using equilibrium solution for drift flux."<<endl;
     #include "driftFluxEE.H"
  }
  else
  {
  
     Info<<"Solving transport equation for drift flux."<<endl;
	 ...
	 + Ysgs
	 - gradYsgs * driftFlux_
	 + fvm::Sp( gradYsgs, driftFlux_ ) 
	 //- fvm::Sp( tauinv * limiterVd, driftFlux_ )
     );
  ...
  {    
     ...
     /*else
     {
	   ...
	   if (uSlip[iCell].z()>=0)
	     {
	       if (driftFlux_[iCell].z() < (0.9*driftFlux_min[iCell].z())) driftFlux_[iCell].z() = 0.9*driftFlux_min[iCell].z();
  ...
  Info<< "min( driftFlux ) = "<< min( driftFlux_ ) << endl
      << "max( driftFlux ) = "<< max( driftFlux_ ) << endl;
}
