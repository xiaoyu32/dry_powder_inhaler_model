...
namespace Foam
{
  namespace dragModels
  {
    ...
    {
      defineTypeNameAndDebug(DriftFluxNNetDM,0);
      addToRunTimeSelectionTable(DriftFluxModel, DriftFluxNNetDM, dictionary);
    }
  }
}
...
Foam::dragModels::driftFluxModels::DriftFluxNNetDM::DriftFluxNNetDM
(
...
:
   ...
   relaxationTime
   (
       dict.subDict("driftFluxModelProps").lookup("relaxationTime")
   ),
   uTerminal
   (
...
void Foam::dragModels::driftFluxModels::DriftFluxNNetDM::evolve()
{
  ...
  */
  ...
  volScalarField relaxationTimeCoefficient = 1/relaxationTime/solidVolumeFraction/solidVolumeFraction/(alphaMax-solidVolumeFraction);
  ...
  (
    ...
    + fvm::div( phiUflux, driftFlux_ )
      ==
      fvm::laplacian( solidVolumeFraction *  driftFluxDiffusivity, driftFlux_ )
      //+ sumXY
   ...
   fvm::ddt( driftFlux_ )
    + fvm::div( phiUflux, driftFlux_ )
      ==
        Xsgs/(1.0-solidVolumeFraction)  
      + Ysgs
   //+ fvm::Sp(relaxationTimeCoefficient,driftFlux_)
  );
