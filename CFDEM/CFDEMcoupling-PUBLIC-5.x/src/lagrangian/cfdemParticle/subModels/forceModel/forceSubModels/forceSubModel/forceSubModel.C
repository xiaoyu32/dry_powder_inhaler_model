    ...
    scaleDH_(dict_.lookupOrDefault<scalar>("scaleDH",1.))
{
	...
	//info about type-specific coarse graining
    if(particleCloud_.cgTypeSpecificDifferent_)
        Info << "WARNING: using type-specific coarse graining (because of setting in your LIGGGHTS input script for 'coarsegraining')! Please ensure you are using the 'getParticleTypes' dict-setting in the 'couplingProperties' such that particle types are available in CFDEM" << endl;
	else
        Info << "type-specific coarse graining not active: will use global coarse graining factor" << endl;
