...
parDEMrun()
{
    ...
    if [[ $machineFileName == "none" ]]; then
    	    ...
    	    srun --ntasks $nrProcs $debugMode $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName
            #mpirun -np $nrProcs $debugMode $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName


    else

        srun -machinefile $machineFileName --ntasks $nrProcs $debugMode $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName
