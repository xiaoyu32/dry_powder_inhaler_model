...
FixPropertyParticle* Modify::add_fix_property_atom(int narg,char **arg,const char *caller)
{
    if(narg < 5) error->all(FLERR,"Not enough arguments to add a fix property");
    add_fix(narg,arg);
    return static_cast<FixPropertyParticle*>(find_fix_property(arg[3],"property/particle",arg[4],0,0,caller));
}
...
/* ----------------------------------------------------------------------
   ...
   find a property registered by a fix property/global or fix property/particle
...
Fix* Modify::find_fix_property(const char *varname,const char *style,const char *svmstyle,int len1,int len2,const char *caller,bool errflag)
{
  ...
  if((strcmp(style,"property/global")) && (strncmp(style,"property/particle",17)))
  {
    fprintf( screen, "Detected style = %s \n", style );
    error->all(FLERR,"Valid styles for find_fix_property are 'property/global' and 'property/particle'");
  }
  ...
  for(ifix = 0; ifix < nfix; ifix++)
  {
      if(strncmp(fix[ifix]->style,"property/particle",13) == 0 && dynamic_cast<FixPropertyParticle*>(fix[ifix]))
         fix_i = static_cast<FixPropertyParticle*>(fix[ifix])->check_fix(varname,svmstyle,len1,len2,caller,errflag);
