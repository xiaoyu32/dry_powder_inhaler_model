...
#define LMP_ASSOCIATIVE_POINTER_ARRAY_I_H
  ...
  // -- extra --
  template<typename T>
  void AssociativePointerArray<T>::updateMesh( const bool* prevIndex, const bool* currentIndex, const int nLocal, const int nGlobal )
  {
        
      for(int i=0;i<numElem_;i++)
         content_[i]->updateMesh( prevIndex, currentIndex, nLocal, nGlobal );
	 	 
  }
  
  template<typename T>
  bool AssociativePointerArray<T>::pushData( char* id, void* tdata, PushWallMPI* push_ )
  {
        
      for(int i=0;i<numElem_;i++)
      {
         if( content_[i]->matches_id( id ) )
	 {
	     content_[i]->pushProps( tdata, push_ );
	     return true;
	 }
      }	 
  ...
  template<typename T>
  bool AssociativePointerArray<T>::pullData( char* id, void* fdata, PullWallMPI* pull_  )
  {
      ...
      for(int i=0;i<numElem_;i++)
      {
         if( content_[i]->matches_id( id ) )
	 {
	     content_[i]->pullProps( fdata, pull_ );
	     return true;
	 }
      }	 
  ...
  template<typename T>
  void AssociativePointerArray<T>::dataSize( char* id, int& len, int&nvec )
  {
      ...
      for(int i=0;i<numElem_;i++)
      {
         if( content_[i]->matches_id( id ) )
	 {
	     len = content_[i]->lenVec();
	     nvec = content_[i]->nVec();
  ...
  template<typename T>
  void AssociativePointerArray<T>::preUpdate()
  {
        
      for(int i=0;i<numElem_;i++)
      {
         bool& update = content_[i]->isUpdated();
	 update = false;
      }
