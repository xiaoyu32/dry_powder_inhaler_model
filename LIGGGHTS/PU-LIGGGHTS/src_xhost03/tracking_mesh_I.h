...
#define LMP_TRACKING_MESH_I_H
  ...
  template<int NUM_NODES>
  void TrackingMesh<NUM_NODES>::deleteFromMap(int n)
  {
      id_.del( n );
  }
  ...
  template<int NUM_NODES>
  void TrackingMesh<NUM_NODES>::updateMesh()
  {
    
    // -- do not update before wallComm is created (happens during restarting)
    if( !this->wallComm ) return;
    
    if( this->sizeGlobal() > id_.capacity() )
    {
        printf( "Capacity overflow tracking_mesh_I.h ! \n" );
	return;
    }
    ...
    // -- mark fields as not updated --
    customValues_.preUpdate();
    
    // -- handled separately --
    bool& id_update = id_.isUpdated();
    id_update = true;
        
    // -- update mesh -- 
    MultiNodeMeshParallel<NUM_NODES>::updateMesh();
          
  }

  template<int NUM_NODES>
  void TrackingMesh<NUM_NODES>::updateMeshProperties()
  {
    // -- update custom values --
    customValues_.updateMesh( this->prevIndex, this->currentIndex, this->nLocal_, this->nGlobal_ );
  }

  template<int NUM_NODES>
  bool TrackingMesh<NUM_NODES>::pushData( char* id, void* tdata )
  {
     
     if( !this->push_ ) return false;   
     
     bool val = MultiNodeMeshParallel<NUM_NODES>::pushData( id, tdata );
     if( val ) return true;
     
     return customValues_.pushData( id, tdata, this->push_ );
     
  }

  template<int NUM_NODES>
  bool TrackingMesh<NUM_NODES>::pullData( char* id, void* fdata )
  {
     if( !this->pull_ ) return false;
     
     bool val = MultiNodeMeshParallel<NUM_NODES>::pullData( id, fdata );
     
     if( val ) return true;
     
     return customValues_.pullData( id, fdata, this->pull_ );
     
  }

  template<int NUM_NODES>
  void TrackingMesh<NUM_NODES>::dataSize( char* id, int& len, int& nvec )
  {  
     MultiNodeMeshParallel<NUM_NODES>::dataSize( id, len, nvec );     
     if( len > 0 || nvec > 0 ) return;
     else customValues_.dataSize( id, len, nvec );
  } 
