...
FixEfieldForce::FixEfieldForce(LAMMPS *lmp, int narg, char **arg) : 
Fix(lmp, narg, arg),
efieldModel_( NULL ),
fix_p( NULL ),
fix_charge( NULL ),
...
FixEfieldForce::~FixEfieldForce()
{
    if( efieldModel_ ) delete[] efieldModel_;
}


void FixEfieldForce::init()
{
    pair_gran = static_cast<PairGran*>(force->pair_match("gran", 0));

    efieldModel_->connectToProperties( force->registry );
    
    fix_charge = static_cast<FixPropertyParticle*>(modify->find_fix_property("charge","property/particle","scalar",0,0,style,false));
...
// -- force calculations --
void FixEfieldForce::post_force(int vflag)
{
  ...
  for(int ii = 0; ii < inum; ii++) 
  {
	...
	const double qj = fix_charge ? fix_charge->vector_atom[j] : 0;
