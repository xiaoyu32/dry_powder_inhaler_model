...
COHESION_MODEL(COHESION_VDW_GU_PARCEL,vdw/gu/parcel,12)
#else
#ifndef COHESION_MODEL_VDW_GU_PARCEL_H_
#define COHESION_MODEL_VDW_GU_PARCEL_H_
...
namespace ContactModels {
  ...
  template<>
  class CohesionModel<COHESION_VDW_GU_PARCEL> : protected Pointers {
  public:
    ...
    {
      if(comm->me==0) printf("COHESION_VDW_GU_PARCEL loaded\n");
    ...
    void connectToProperties(PropertyRegistry & registry) {      
      ...
      fix_nparcel = static_cast<FixPropertyGlobal*>(modify->find_fix_property("nparcel","property/particle","peratomtype",max_type,0,"vdw_gu_parcel"));

      if( !fix_nparcel ) error->all(FLERR,"cohesion_model_vdw_gu_parcel requires property/particle fix 'nparcel'!");
	...
	if(cdata.is_wall) {
      ...
      // -- parcel contribution --
      // Fn_coh *= cdata.is_wall ? nparcel[itype-1] : nparcel[itype-1] * nparcel[jtype-1];
      
      Fn_coh *= cdata.is_wall ? nparcel[itype-1] : max(nparcel[itype-1], nparcel[jtype-1]);
