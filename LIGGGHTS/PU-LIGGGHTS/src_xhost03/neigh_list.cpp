...
void NeighList::post_constructor(NeighRequest *nq)
{
  ...
  copy = nq->copy;
  dnum = nq->dnum; // -- LIGGHTS -- (JK)
  
  
  // -- LIGGHTS --
  if( nq->granhistory )
  {  
     neighbor->lists[this->index-1]->listgranhistory = this;
     
     for (int ifix = 0; ifix < modify->nfix; ifix++){
        if (strcmp(modify->fix[ifix]->style,"contacthistory") == 0) 
	{  
          this->fix_history = dynamic_cast<FixContactHistory*>(modify->fix[ifix]); 
        }
     }     
...
void NeighList::setup_pages(int pgsize_caller, int oneatom_caller, int dnum_caller )
{
