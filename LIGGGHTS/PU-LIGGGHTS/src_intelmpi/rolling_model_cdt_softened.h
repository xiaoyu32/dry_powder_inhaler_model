...
------------------------------------------------------------------------- */
#ifdef ROLLING_MODEL
ROLLING_MODEL(ROLLING_CDT_SOFTENED,cdt_softened,4)
#else
#ifndef ROLLING_MODEL_CDT_SOFTENED_H_
#define ROLLING_MODEL_CDT_SOFTENED_H_
...
namespace ContactModels
{
  ...
  template<>
  class RollingModel<ROLLING_CDT_SOFTENED> : protected Pointers {
  public:
    ...
    void connectToProperties(PropertyRegistry & registry)
    {
      ...
      registry.connect("coeffRollFrict", coeffRollFrict,"rolling_model cdt_softened");

      // error checks on coarsegraining
      if(force->cg_active())
        error->cg(FLERR,"rolling model cdt_softened");
    }
    ...
    void collision(CollisionData & cdata, ForceData & i_forces, ForceData & j_forces) 
    {
      ...
      if(cdata.is_wall){
        ...
        if(wr_rollmag > 0.)
        {
          ...
          vectorScalarMult3D(wr_roll,chiStiff*rmu*cdata.kn*cdata.deltan*reff/wr_rollmag,r_torque);
...
#endif // ROLLING_MODEL_CDT_SOFTENED_H_
#endif
