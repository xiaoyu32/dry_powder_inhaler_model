...
int Neighbor::init_pair()
{
  ...
  for (i = 0; i < nlist; i++) {
    ...
    lists[i]->setup_pages(pgsize,oneatom,requests[i]->dnum); // -- LIGGHTS modification -- (JK)
  }
...
void Neighbor::morph_copy()
{
  ...
  for (i = 0; i < nrequest; i++) {
    ...
    for (j = 0; j < nrequest; j++) {
      ...
      //FIXME: should add the dnum here? (JK)
      if( irq->dnum != jrq->dnum ) continue;
...
void Neighbor::build(int topoflag)
{
  ...
  if (style != Neighbor::NSQ) 
  {
    ...
    //if( proc_id == 0 ) printf( "Particle binning completed. \n" ); 
  }
