...
//#include <iostream>
  ...
  MultiNodeMeshParallel<NUM_NODES>::~MultiNodeMeshParallel()
  {
      ...
      if( nodeCopy_ )   delete nodeCopy_;
      if( centerCopy_ ) delete centerCopy_;
      if( rBoundCopy_ ) delete rBoundCopy_;
      
      if( currentIndex ) delete currentIndex;
      if( prevIndex )    delete prevIndex;
      
      // -- delete cfd coupling fixes --
      if( wallComm ) delete wallComm;
      if( pull_ ) delete pull_;
      if( push_ ) delete push_;
  }
   
  template<int NUM_NODES>
  int MultiNodeMeshParallel<NUM_NODES>::ncfd() const
  {  
  
      //if( wallComm ) printf( "ncfd() = %d \n", wallComm->ncfd() );
  
      if( !wallComm || !wallComm->isActive() ) return 0;
      else return wallComm->ncfd();
  } 
   

  template<int NUM_NODES>
  void MultiNodeMeshParallel<NUM_NODES>::dataSize( char* id, int& len, int& nvec )
  {
          
     AbstractMesh::dataSize( id, len, nvec );
               
     if( this->center_.matches_id( id ) )
     {
        len = 3;
	nvec = 1;
	return;
  ...
  /*template<int NUM_NODES>
  void MultiNodeMeshParallel<NUM_NODES>::deleteElement2(int n)
  {
    ...
    //printf( "I)  deleteElement(int n) n = %d    this->center_.size() = %d %d %d \n", n, this->center_.size(), this->rBound_.size(), this->node_.size() );
    //printf( "II)  deleteElement(int n) n = %d    this->center_.size() = %d %d %d \n", n, this->center_.numElem_, this->rBound_.numElem_, this->node_.numElem_ );
        
    this->node_.del(n);
    if(this->node_orig_) this->node_orig_->del(n);
    this->center_.del(n);    
    this->rBound_.del(n);    
  ...
  bool MultiNodeMeshParallel<NUM_NODES>::allNodesInsideSimulationBox()
  {
    ...
    for(int i=0;i<sizeLocal();i++)
      for(int j=0;j<NUM_NODES;j++)
      {
        ...
        //if(!this->domain->is_in_domain(this->node_(i)[j])) FIXME: might need some extra work!
        if(!this->domain->inside(this->node_(i)[j]))
	{
            flag = 1;
            break;
        }
...
------------------------------------------------------------------------- */
  ...
  /* ----------------------------------------------------------------------
   parallelization -
   refresh mesh, called after changing processor boundaries; FIXME: send only the elements needed by the receiving processor
  ------------------------------------------------------------------------- */  

  template<int NUM_NODES>
  bool MultiNodeMeshParallel<NUM_NODES>::pushData( char* id, void* tdata )
  {
     
     if( !push_ ) return false;
     
     if( this->center_.matches_id( id ) )
     {
        this->center_.pushProps( tdata, push_ );
	return true;
     }
     
     if( this->node_.matches_id( id ) )
     {
        this->node_.pushProps( tdata, push_ ); 
	return true;
     }
  ...
  template<int NUM_NODES>
  bool MultiNodeMeshParallel<NUM_NODES>::pullData( char* id, void* fdata )
  {
     if( !pull_ ) return false;
     
     if( this->center_.matches_id( id ) )
     {
        this->center_.pullProps( fdata, pull_ );
	return true;
     }
     
     if( this->node_.matches_id( id ) )
     {
        this->node_.pullProps( fdata, pull_ ); 
	return true;
     }
  ...
  template<int NUM_NODES>
  void MultiNodeMeshParallel<NUM_NODES>::updateMesh()
  {
     
     if( !wallComm ) return;
     
     AbstractMesh::updateMesh();
     
     if( !nodeCopy_ || !centerCopy_ || !rBoundCopy_ )
     {
        printf( "updateMesh() called before pointers allocated !!!!!!!!!!!!! \n" );
        return;
     }
     ...
     // -- reinitialize the mesh fields --
     this->node_.shallowCopy( *nodeCopy_ );
     this->center_.shallowCopy( *centerCopy_ );
     this->rBound_.shallowCopy( *rBoundCopy_ );

     // -- copy the full mesh to the active mesh lists --
     this->nGlobal_ = this->node_.size();    //this->node_.size();
     this->nLocal_ = this->node_.size();
     this->nGhost_ = 0;
     
     // -- nLocal_ is not same as "nGlobal_" --
     for( int i = 0; i < nLocal_; ++i )
          this->currentIndex[i] = true;

	  
     // -- delete elements that do not belong to the processor, update nGlobal_, update currentIndex --
     deleteUnownedUpdate();
     
     this->updateMeshProperties();
          
     // -- udpate prevIndex array to currentIndex --
     for( int i = 0; i < this->nGlobal_; ++i )
     {
	this->prevIndex[i] = this->currentIndex[i];
     } 
     
     // -- update the processor domain information --
     setup();  
     refreshOwned(0); 
     borders(); 
     refreshGhosts(0);
     ...
     // -- coupling active, initialize wallComm --
     if( wallComm->isActive() ) wallComm->init( this->center_.begin(), this->nLocal_ );
      
  }


  template<int NUM_NODES>
  void MultiNodeMeshParallel<NUM_NODES>::constructCopy()
  {
  
     if( nodeCopy_ || centerCopy_ || rBoundCopy_ ) return; // -- copies already exist --
    
     // -- invoke copy constructor --
     nodeCopy_ =   new MultiVectorContainer<double,NUM_NODES,3>( this->node_ );
     centerCopy_ = new VectorContainer<double,3>( this->center_ ); 
     rBoundCopy_ = new ScalarContainer<double>( this->rBound_ );      
 
     if( !prevIndex ) prevIndex =       new bool[ this->node_.size() ];
     if( !currentIndex ) currentIndex = new bool[ this->node_.size() ];
     ...
     //printf( "Processor %d : %d    %d    %d ", id, nodeCopy_->size(), centerCopy_->size(), rBoundCopy_->size()  );
  ...
  void MultiNodeMeshParallel<NUM_NODES>::initalSetup()
  {
      ...
      // -- save copies for mesh updates (for cases where processor domains chance)
      constructCopy();
      ...
      if( !wallComm )
      {
         wallComm = new WallComm();
	 pull_ = new PullWallMPI( wallComm );
	 push_ = new PushWallMPI( wallComm );
	 
	 wallComm->setActive( wallCommActive_ );
	 
	 if( wallComm->isActive() )
	 {
             wallComm->init( this->center_.begin(), this->nLocal_ );
	 }
  ...
  ------------------------------------------------------------------------- */
  template<int NUM_NODES>
  void MultiNodeMeshParallel<NUM_NODES>::deleteUnownedUpdate()
  {
      ...
      if(doParallellization_)
      {
	      ...
	      if( !this->domain->is_in_subdomain( this->center_(i) ) )
              {    	      
		  MultiNodeMeshParallel<NUM_NODES>::deleteElement(i); // -- do not delete the properties yet --		  
		  this->deleteFromMap(i); // -- delete matching id --		  
		  currentIndex[index] = false; // -- mark the face false --		  		  
              }else i++;
  ...
  void MultiNodeMeshParallel<NUM_NODES>::deleteUnowned()
  {
      ...
      if(doParallellization_)
      {
	      ...
	      if( !this->domain->is_in_subdomain( this->center_(i) ) )
              {      
	          //MultiNodeMeshParallel<NUM_NODES>::deleteElement(i);
