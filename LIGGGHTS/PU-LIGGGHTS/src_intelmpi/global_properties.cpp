...
namespace MODEL_PARAMS
{
  ...
  static const char * CAP_DISTANCE_VISCOUS_FORCE = "capDistanceViscousForce";
  
  //lubrication force parameters
  static const char * CUTOFF = "cutOff";
  
  //triboelectricity parameters
  static const char * PERMITTIVITY = "permittivity";
  static const char * SCREENINGDISTANCE = "screeningDistance";
  static const char * DELTA_CHARGE = "deltaCharge";
  static const char * BD_FIELD = "bd_field";
  
  // Gu cohesion model parameters
  static const char * S_MIN = "sMin";
  static const char * S_MINS = "sMins";
  static const char * S_O = "sO";
  /* -----------------------------------------------------------------------
   * Utility functions
   * ----------------------------------------------------------------------- */
...
//-AO-08/052015 
  ...
  ScalarProperty* createCapDistanceViscousForce(PropertyRegistry & registry, const char * caller, bool)
  {
    return createScalarProperty(registry, CAP_DISTANCE_VISCOUS_FORCE, caller);
  }  
  /* ---------------------------------------------------------------------- */

  // -- vacuum permittivity -- JK
  ScalarProperty* createPermittivity(PropertyRegistry & registry, const char * caller, bool)
  {
    return createScalarProperty(registry, PERMITTIVITY, caller);
  }
  
  // -- screening distance for P3M -- JK
  ScalarProperty* createScreeningDistance(PropertyRegistry & registry, const char * caller, bool)
  {
    return createScalarProperty(registry, SCREENINGDISTANCE, caller);
  }
  
  
  //lubrication model properties (JK)
  ScalarProperty* createCutOff(PropertyRegistry & registry, const char * caller, bool)
  {
    return createScalarProperty(registry, CUTOFF, caller);
  }
  
  //breakdown field
  ScalarProperty* createBdfield(PropertyRegistry & registry, const char * caller, bool)
  {
    return createScalarProperty(registry, BD_FIELD, caller);
  }
   

  // Gu cohesion model parameters  
  MatrixProperty* createSMin(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, S_MIN, caller);
  }

  MatrixProperty* createSMins(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, S_MINS, caller);
  }

  MatrixProperty* createSO(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, S_O, caller);
  }  
