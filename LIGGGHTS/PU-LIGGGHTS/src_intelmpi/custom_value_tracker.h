...
namespace LAMMPS_NS
{
  class CustomValueTracker : protected Pointers
  {
	...
	inline void dataSize( char* id, int& len, int&nvec ){ elementProperties_.dataSize( id, len, nvec ); }
	
	inline bool pushData( char* id, void* tdata, PushWallMPI* push_ ){ return elementProperties_.pushData( id, tdata, push_ ); }
	inline bool pullData( char* id, void* fdata, PullWallMPI* pull_ ){ return elementProperties_.pullData( id, fdata, pull_ ); }
	
	void updateMesh( const bool*, const bool*, const int, const int );
	void preUpdate();
