...
namespace LAMMPS_NS
{
  ...
  class MultiNodeMeshParallel : public MultiNodeMesh<NUM_NODES>
  {
	...
	virtual void updateMeshProperties() = 0;

	virtual bool pushData( char* id, void* tdata );
	virtual bool pullData( char* id, void* fdata );
	
	virtual void dataSize( char* id, int& len, int&nvec );
	virtual int ncfd() const;
	...
	virtual void setActive( bool wallCommActive )
	{
	    wallCommActive_ = wallCommActive;
	}
	...
	// -- boolean indicating if wall communication is needed --
	bool wallCommActive_;
	...
	virtual void deleteFromMap(int n) = 0;
	...
	//
	MultiVectorContainer<double,NUM_NODES,3>* nodeCopy_;
        VectorContainer<double,3>* centerCopy_; 
        ScalarContainer<double>* rBoundCopy_; 
	...
	void deleteUnownedUpdate();
	
	//void deleteElement2(int);
