...
void FixBalance::setup_pre_exchange()
{
  ...
  { 
     ...
     // -- update the mesh (if exist) --
     FixMesh* meshFix = NULL;
     int rank = 0;
          
     do
     {
	meshFix = static_cast<FixMesh*>( modify->find_fix_style("mesh/surface", rank) );
	...
	++rank;
     ...
     /*FixEulerianCPUs* fixCpus = static_cast<FixEulerianCPUs*>( modify->find_fix_style("cpus", 0) );
...
void FixBalance::pre_exchange()
{
  ...
  { 
     ...
     if( proc_id == 0 ) fprintf( screen, "Rebalancing particles \n" );
     rebalance(); // -- rebalance --
	...
	if( proc_id == 0 ) fprintf( screen, "Updated mesh %d \n", rank );
	
	++rank;
	
	//bool foobar = meshFix ? true : false;
	
	//fprintf( screen, "END OF STEP %d \n", foobar ); //FIXME
	//MPI_Barrier( MPI_COMM_WORLD );
	
     }while( meshFix );
     
     if( proc_id == 0 ) fprintf( screen, "Meshes Updated \n" );
