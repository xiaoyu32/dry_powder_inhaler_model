...
class FixWallGran : public Fix, public LIGGGHTS::IContactHistorySetup {
 ...
 protected:
  ...
  virtual void compute_scalar_transport( LCM::CollisionData & cdata, double * v_wall, int iMesh, FixMeshSurface *fix_mesh, TriMesh *mesh, int iTri );
  ...
  // heat transfer
  class FixPropertyParticle *fppa_T;
  class FixPropertyParticle *fppa_hf;
  ...
  class FixPropertyParticle *fix_history_primitive_;
  ...
  // force storage
  bool store_force_;
  class FixPropertyParticle *fix_wallforce_;

  // max neigh cutoff - as in Neighbor
  double cutneighmax_;
  
  // mesh created during restart?
  bool restart_flag;
  ...
  inline void post_force_eval_no_contact(LCM::CollisionData & cdata, double * v_wall, int iMesh = -1, FixMeshSurface *fix_mesh = 0, TriMesh *mesh = 0, int iTri = 0);
};
