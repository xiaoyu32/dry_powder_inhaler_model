...
using namespace LAMMPS_NS;
     ...
     useDefault_(orig.useDefault_)
  {
      if(orig.id_)
      {
        id_ = new char[strlen(orig.id_)+1];
        strcpy(id_,orig.id_);
      }
  }
  ...
  bool ContainerBase::matches_id(const char *_id)
  {      
        
      if( !this->id_ ) 
      {
         printf( "Null ID detected!!!!!!!!!!!!!!!!!!!!!!!!! \n" );
         return false;
      }
      
      if( !_id ) return false;
      if( strlen( _id ) != strlen( this->id_ ) ) return false;	
      if( strcmp(_id, this->id_) == 0) return true;
      return false;
