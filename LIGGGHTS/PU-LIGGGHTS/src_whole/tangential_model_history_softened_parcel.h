...
TANGENTIAL_MODEL(TANGENTIAL_HISTORY_SOFTENED_PARCEL,history_softened_parcel,5)
#else
#ifndef TANGENTIAL_MODEL_HISTORY_SOFTENED_PARCEL_H_
#define TANGENTIAL_MODEL_HISTORY_SOFTENED_PARCEL_H_
...
{
  template<>
  class TangentialModel<TANGENTIAL_HISTORY_SOFTENED_PARCEL> : protected Pointers
  {
    double ** coeffFrict;
    int history_offset;
  ...
  public:
	...
	shear[0] += cdata.vtr1 * dt;
      ...
      double Fn = cdata.is_wall ? cdata.Fn/nparcel[itype-1] : cdata.Fn/max(nparcel[itype-1], nparcel[jtype-1]);
      const double Ft_friction = xmu * chiStiff * fabs(Fn);
