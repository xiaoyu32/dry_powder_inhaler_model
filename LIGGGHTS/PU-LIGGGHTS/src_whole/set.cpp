...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     SMD_CONTACT_RADIUS,DPDTHETA,INAME,DNAME,VX,VY,VZ,PROPERTYPERPARTICLE};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
    ...
    } else if (strncmp(arg[iarg],"property/particle",13) == 0) { 
      if (iarg+1 > narg)
        error->all(FLERR,"Illegal set command for property/particle");
      ...
      for (int ifix = 0; ifix < (lmp->modify->nfix); ifix++){
        if ((strncmp(modify->fix[ifix]->style,"property/particle",13) == 0) && (strcmp(((FixPropertyParticle*)(modify->fix[ifix]))->variablename,variablename)==0) ){
            updFix=(FixPropertyParticle*)(lmp->modify->fix[ifix]);
        }
      }
      delete []variablename;
      if (updFix==NULL)
        error->all(FLERR,"Could not identify the per-particle property you want to set");
      nUpdValues=updFix->nvalues;

      if (nUpdValues != (narg-iarg-2) )
        error->all(FLERR,"The number of values for the set property/particle does not match the number needed");

      //get to update values
      if (strstr(arg[iarg+2],"v_") == arg[iarg+2])
      {
        if (nUpdValues != 1 && nUpdValues != 3)
            error->all(FLERR,"Set command for property/particle and variables does only work with scalar and 3-vector quantities");
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    // set desired per-particle property
    else if (keyword == PROPERTYPERPARTICLE) { 

        // if fix was just created, its default values have not been set,
        // so have to add a run 0 to call setup
        if(updFix->just_created)
            error->one(FLERR,"May not use the set command right after fix property/particle without a prior run. Add a 'run 0' between fix property/particle and set");
