...
#define LARGE_TRIMESH 1000000
                                   ...
                                   double *cSphere, double *delta, double *bary)
  {
    ...
    int barySign = (bary[0] > -EPSILON_PRECISION*invlen) + 2*(bary[1] > -EPSILON_PRECISION*invlen) + 4*(bary[2] > -EPSILON_PRECISION*invlen);
  ...
  inline bool TriMesh::resolveTriSphereNeighbuild
        (
	...
	)
  {
    ...
    double dNorm = fabs( 
    		   calcDistToPlane(
			 	    cSphere,
        		 	    SurfaceMeshBase::center_(nTri),
			 	    SurfaceMeshBase::surfaceNorm(nTri)
				  ) 
		       );
