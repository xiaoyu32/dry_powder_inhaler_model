...
void NBinStandard::setup_bins(int style)
{
  ...
  if (bbin > MAXBIGINT) error->one(FLERR,"Too many neighbor bins"); // (JK), hash table supports 64 bit keys
  
  mbins = bbin;
  
  //printf( "nx = %d   ny = %d    nz = %d    mx = %d   my = %d    mz = %d \n", nbinx, nbiny, nbinz, mbinx, mbiny, mbinz ); //fixme
...
void NBinStandard::bin_atoms()
{
  ...
  //for (i = 0; i < mbins; i++) binhead[i] = -1; // -- not needed -- (JK)
