...
void FixContactHistory::pre_exchange()
{
  ...
  //printf( screen,"FixContactHistory::pre_exchange() \n" );
  ...
  //if( list->fix_history ) fprintf( screen, "history fix exists! \n" );
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    //fprintf( screen, "jnum = %d \n", jnum );
    
    for (jj = 0; jj < jnum; jj++) {
      
      //fprintf( screen,"touch[%d] = %d \n", jj, touch[jj] );
  ...
  /*if( (dnum_+1)*maxtouch_+1 > comm->maxexchange_fix )
  {
      fprintf( screen, "FixContactHistory increased maxexchange_fix to %d from %d  (dnum = %d     maxtouch = %d )\n", (dnum_+1)*maxtouch_+1, comm->maxexchange_fix, dnum_, maxtouch_ ); //FIXME
  }*/
