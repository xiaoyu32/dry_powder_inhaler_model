...
FixGravityParcel::FixGravityParcel(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  mstr(NULL), vstr(NULL), pstr(NULL), tstr(NULL), xstr(NULL), ystr(NULL), zstr(NULL)
{
  if (narg < 5) error->all(FLERR,"Illegal fix gravity parcel command");
  ...
  if (strcmp(arg[4],"chute") == 0) {
    if (narg != 6) error->all(FLERR,"Illegal fix gravity parcel command");
    style = CHUTE;
    ...
    if (narg != 7) error->all(FLERR,"Illegal fix gravity parcel command");
    style = SPHERICAL;
    if (strstr(arg[5],"v_") == arg[5]) {
      int n = strlen(&arg[5][2]) + 1;
      pstr = new char[n];
    ...
    if (narg != 8) error->all(FLERR,"Illegal fix gravity parcel command");
    style = VECTOR;
    if (strstr(arg[5],"v_") == arg[5]) {
      int n = strlen(&arg[5][2]) + 1;
      xstr = new char[n];
  ...
  } else error->all(FLERR,"Illegal fix gravity parcel command");
...
FixGravityParcel::~FixGravityParcel()
{
  if (copymode) return;
...
void FixGravityParcel::init()
{
  ...
  if (mstr) {
    ...
    if (mvar < 0)
      error->all(FLERR,"Variable name for fix gravity parcel does not exist");
    if (!input->variable->equalstyle(mvar))
      error->all(FLERR,"Variable for fix gravity parcel is invalid style");
  }
  if (vstr) {
  ...
  fix_nparcel = static_cast<FixPropertyGlobal*>(modify->find_fix_property("nparcel","property/particle","peratomtype",max_type,0,"gravity/parcel"));

    if( !fix_nparcel ) error->all(FLERR,"Fix 'gravity/parcel' requires property/particle fix 'nparcel'!");

    nparcel = new double[max_type];
    
    for( int i = 0; i < max_type; ++i )
    {
    	if( fix_nparcel ) nparcel[i] = fix_nparcel->compute_vector(i);
	else nparcel[i] = 1.0;
...
void FixGravityParcel::setup(int vflag)
{
...
void FixGravityParcel::post_force(int /*vflag*/)
{
  ...
  if (rmass) {
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        massone = rmass[i];
	fgrav[1] = massone*xacc;
	fgrav[2] = massone*yacc;
	fgrav[3] = massone*zacc;
	
	// parcel contribution
	fgrav[1] *= nparcel[type[i]-1];
	fgrav[2] *= nparcel[type[i]-1];
	fgrav[3] *= nparcel[type[i]-1];
...
void FixGravityParcel::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixGravityParcel::set_acceleration()
{
...
double FixGravityParcel::compute_scalar()
{
