...
void CommBrick::exchange()
{
  ...
  //fprintf( screen, "CommBrick::exchange() (0) \n" ); //FIXME
  ...
  {
     //fprintf( screen, "CommBrick::exchange() maxexchange = %d    maxexchange_atom = %d maxexchange_fix = %d BUFEXTRA = %d \n", maxexchange, maxexchange_atom, maxexchange_fix, BUFEXTRA ); //FIXME
     //fprintf( screen, "CommBrick::exchange() maxsend = %d    bufextra = %d \n", maxsend, bufextra ); //FIXME
  ...
  for (int dim = 0; dim < dimension; dim++) {
    ...
    while (i < nlocal) {
      if (x[i][dim] < lo || x[i][dim] >= hi) {
        if (nsend > maxsend){
	  //fprintf( screen, "CommBrick::exchange() maxsend = %d    nsend = %d  !! (here) \n", maxsend, nsend ); //FIXME
	  grow_send(nsend,1);
	}
...
void CommBrick::borders()
{
  ...
  for (dim = 0; dim < 3; dim++) {
    ...
    for (ineed = 0; ineed < twoneed; ineed++) {
      ...
      if (nsend*size_border > maxsend){
        //fprintf( screen, "CommBrick::exchange() size_border = %d    nsend = %d  !!! \n", size_border, nsend ); //FIXME
        ...
        if (nrecv)
	{
	   MPI_Irecv(buf_recv,nrecv*size_border,MPI_DOUBLE,
                             recvproc[iswap],0,world,&request); // no prob from unsyncronizing
	   
	}		     
			     
        if (n){
	   MPI_Send(buf_send,n,MPI_DOUBLE,sendproc[iswap],0,world);
	   //MPI_Send(buf_send,nsend*size_border,MPI_DOUBLE,sendproc[iswap],0,world);
	}
  ...
  if (max > maxsend){ 
    //fprintf( screen, "CommBrick::exchange() max = %d    maxsend = %d  !!!! \n", max, maxsend ); //FIXME 
    grow_send(max,0);
  }
...
void CommBrick::reverse_comm_fix_variable(Fix *fix)
{
  ...
  for (iswap = nswap-1; iswap >= 0; iswap--) {
    ...
    if (nsend > maxsend){
    //fprintf( screen, "CommBrick::reverse_comm_fix_variable(Fix *fix) nsend = %d    maxsend = %d  !!!!! \n", nsend, maxsend ); //FIXME 
...
void CommBrick::forward_comm_array(int nsize, double **array)
{
  ...
  if (nsize > maxforward) {
    ...
    if (maxforward*smax > maxsend){
         //fprintf( screen, "CommBrick::forward_comm_array( maxforward = %d    smax = %d  !!!!!! \n", maxforward, smax ); //FIXME 
