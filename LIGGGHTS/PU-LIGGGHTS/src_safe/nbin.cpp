...
NBin::NBin(LAMMPS *lmp) : Pointers(lmp)
{
  last_bin = -1;
  maxbin = maxatom = 0;
  //binhead = NULL; (JK, not needed for hash table)
  bins = NULL;
  atom2bin = NULL;
...
NBin::~NBin()
{
  //memory->destroy(binhead); -- binhead is statically reserved, no destroy needed --
  memory->destroy(bins);
...
void NBin::bin_atoms_setup(int nall)
{
  ...
  // -- reserve space for bins & clear the memory -- (there will be max nall unempty bins)
  binhead.reserve( nall );
...
long NBin::coord2bin(double *x)
{
  ...
  return long(iz-mbinzlo)*long(mbiny*mbinx) + long(iy-mbinylo)*long(mbinx) + long(ix-mbinxlo);
}

long NBin::coord2bin(double *x, int &ix, int &iy, int &iz)
{
  if (x[0] >= bboxhi[0])
    ix = static_cast<int> ((x[0]-bboxhi[0])*bininvx) + nbinx;
  else if (x[0] >= bboxlo[0]) {
  ...
  iy -= mbinylo;
  iz -= mbinzlo;
  return long(iz)*long(mbiny*mbinx) + long(iy*mbinx) + long(ix);
}

void NBin::bin_center(int ix, int iy, int iz, double * center)
{
  center[0] = (mbinxlo + ix + 0.5) * binsizex + bboxlo[0];
  center[1] = (mbinylo + iy + 0.5) * binsizey + bboxlo[1];
  center[2] = (mbinzlo + iz + 0.5) * binsizez + bboxlo[2];
}
