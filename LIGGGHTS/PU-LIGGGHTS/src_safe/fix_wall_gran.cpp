  ...
  Fix(lmp, narg, arg)
{
	...
	// XL
	else if (strcmp(arg[iarg_],"restart") == 0) {
           if (iarg_+2 > narg)
              error->fix_error(FLERR,this," not enough arguments");
           if (strcmp(arg[iarg_+1],"yes") == 0) restart_flag = true;
           else if (strcmp(arg[iarg_+1],"no") == 0) restart_flag = false;
           else error->fix_error(FLERR,this,"expecting 'yes' or 'no' after keyword 'restart'");
           hasargs = true;
           iarg_ += 2;
        } else {
	  ++iarg_;
...
void FixWallGran::post_create()
{
    ...
    if(store_force_)
    {
          ...
          fix_wallforce_ =
              static_cast<FixPropertyParticle*>(modify->find_fix_property(wallforce_name,"property/particle","vector",3,0,style));
          delete []fixarg;
   ...
   if(meshwall_ == 0 && dnum_ > 0)
   {
          ...
          fix_history_primitive_ =
              static_cast<FixPropertyParticle*>(modify->find_fix_property(hist_name,"property/particle","vector",dnum_,0,style));
          delete []fixarg;
          delete []hist_name;
   }
}
    ...
    if(strncmp(style,"wall/gran",9) == 0)
    {
        ...
        if(is_mesh_wall())
        {
            ...
            for (int ifix = 0; ifix < nfix; ifix++)
            {
                ...
                if (	fwg->is_mesh_wall() && 
			strcmp(fwg->style, "wall/gran") == 0 &&
			strcmp(this->style, "wall/gran") == 0 )
...
void FixWallGran::setup(int vflag)
{
    ...
    if (strstr(update->integrate_style,"verlet"))
    {
      ...
      pre_neighbor();
      pre_force(vflag);
      post_create(); // -- LIGGHTS -- probably not needed anymore (JK)
      post_force(vflag);
...
void FixWallGran::post_force_mesh(int vflag)
{
    ...
    for(int iMesh = 0; iMesh < n_FixMesh_; iMesh++)
    {
      ...
      else
      {
        ...
        for(int iTri = 0; iTri < nTriAll; iTri++)
        {
          ...
          for(int iCont = 0; iCont < numneigh; iCont++)
          {
	       ...
	       post_force_eval_no_contact(cdata, v_wall,iMesh,FixMesh_list_[iMesh],mesh,iTri);
      ...
      if(fix_contact && !restartFlag()) fix_contact->cleanUpContacts();
...
void FixWallGran::post_force_eval_no_contact(CollisionData & cdata, double * v_wall, int iMesh, FixMeshSurface *fix_mesh, TriMesh *mesh, int iTri)
{

  //JK: added this for scalar transport between walls
  if( mesh )
    compute_scalar_transport( cdata, v_wall, iMesh, fix_mesh, mesh, iTri );
}
...
void FixWallGran::compute_scalar_transport( CollisionData& cdata, double * v_wall, int iMesh, FixMeshSurface *fix_mesh, TriMesh *mesh, int iTri )
{
    fix_mesh->compute_scalar_transport( cdata, v_wall, iMesh, iTri );
}
...
inline void FixWallGran::post_force_eval_contact(CollisionData & cdata, double * v_wall, int iMesh, FixMeshSurface *fix_mesh, TriMesh *mesh, int iTri)
{
  ...
  //cdata.r = r0_ - cdata.deltan; //XL: test
  ...
  if(cwl_ && addflag_)
  {
   ...
   //JK: changed this out of else
  compute_force(cdata, v_wall); // LEGACY CODE (SPH)

  
  //JK: added this for scalar transport between walls & mesh based force calculations
  if( mesh )
    compute_scalar_transport( cdata, v_wall, iMesh, fix_mesh, mesh, iTri );
...
void FixWallGran::init_heattransfer()
{
    ...
    fppa_T = static_cast<FixPropertyParticle*>(modify->find_fix_property("Temp","property/particle","scalar",1,0,style));
    fppa_hf = static_cast<FixPropertyParticle*>(modify->find_fix_property("heatFlux","property/particle","scalar",1,0,style));
