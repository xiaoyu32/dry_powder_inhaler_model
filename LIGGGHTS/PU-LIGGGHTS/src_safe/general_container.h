...
namespace LAMMPS_NS
{
  ...
  class GeneralContainer : public ContainerBase
  {
	  ...
	  virtual void updateMesh( const bool*, const bool*, const int nLocal, const int nGlobal );
	  virtual void pullProps( void*& values, PullWallMPI* pull_ ); // of to liggghts
	  virtual void pushProps( void*& values, PushWallMPI* push_ ); // liggghts to of
	  ...
	  // -- creates a shallow copy to orig
	  void shallowCopy( GeneralContainer<T,NUM_VEC,LEN_VEC> const &orig );
