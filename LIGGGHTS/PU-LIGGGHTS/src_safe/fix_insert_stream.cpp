...
{
  // JK: does not appear to do anything!
...
void FixInsertStream::init()
{
    ...
    fix_release = static_cast<FixPropertyParticle*>(modify->find_fix_property("release_fix_insert_stream","property/particle","vector",5,0,style));
...
{
    if((!domain->inside(ins_vol_xmin) || !domain->inside(ins_vol_xmax)) && comm->me == 0)
