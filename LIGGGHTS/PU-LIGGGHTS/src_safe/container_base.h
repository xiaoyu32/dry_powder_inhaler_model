...
namespace LAMMPS_NS
{
  ...
  class ContainerBase
  {
	  ...
	  inline const char* getId() const;
          ...
          bool matches_id(const char *_id);

          virtual bool isDoubleData() = 0;
          virtual bool isIntData() = 0;
	  
	  virtual void updateMesh( const bool*, const bool*, const int, const int ) = 0;
	  
	  virtual void pullProps( void*& values, class PullWallMPI* pull_ ){}; // of to liggghts
	  virtual void pushProps( void*& values, class PushWallMPI* push_ ){};  // liggghts to of
