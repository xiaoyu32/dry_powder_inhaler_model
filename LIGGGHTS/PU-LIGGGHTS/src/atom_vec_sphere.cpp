...
AtomVecSphere::AtomVecSphere(LAMMPS *lmp) : AtomVec(lmp)
{
  ...
  //FIXME: other sizes may need adjustment due to introduction of density field (JK)
  ...
  size_reverse = 6;
  //size_border = 8;
  size_border = 9; // -- added density field -- (JK)
...
void AtomVecSphere::grow(int n)
{
  ...
  density = memory->grow(atom->density,nmax,"atom:density");  // -- LIGGHTS -- (JK)
...
void AtomVecSphere::create_atom(int itype, double *coord)
{
  ...
  rmass[nlocal] = 4.0*MY_PI/3.0 * radius[nlocal]*radius[nlocal]*radius[nlocal]* density[nlocal];
  omega[nlocal][0] = 0.0;
  omega[nlocal][1] = 0.0;
  omega[nlocal][2] = 0.0;
