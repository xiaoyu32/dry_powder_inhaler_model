...
FixPropertyParticle::FixPropertyParticle(LAMMPS *lmp, int narg, char **arg, bool parse) :
  Fix(lmp, narg, arg),
  propertyname(0),
  property(0)
{
...
void FixPropertyParticle::parse_args(int narg, char **arg)
{
    // Check args
...
//-AO
    else if (strcmp(arg[4],"matrix") == 0) data_style = FIXPROPERTY_ATOM_VECTOR;
//-AO
...
//-AO
    if (strcmp(arg[4],"matrix") == 0) nvalues = 9; 
//-AO
    ...
    for (int ifix = 0; ifix < modify->nfix; ifix++)
        if ((modify->fix[ifix]) && (strcmp(modify->fix[ifix]->style,style) == 0) && (strcmp(((FixPropertyParticle*)(modify->fix[ifix]))->variablename,variablename)==0) )
...
FixPropertyParticle::~FixPropertyParticle()
{
...
Fix* FixPropertyParticle::check_fix(const char *varname,const char *svmstyle,int len1,int len2,const char *caller,bool errflag)
{
    char errmsg[400];
...
int FixPropertyParticle::setmask()
{
  int mask = 0;
  mask |= PRE_EXCHANGE;
  return mask;
...
void FixPropertyParticle::do_forward_comm()
{
    //timer->stamp(); //FIXME: add the time spent
    if (commGhost) comm->forward_comm_fix(this);
    else error->all(FLERR,"FixPropertyParticle: Faulty implementation - forward_comm invoked, but not registered");
    //timer->stamp(TIME_COMM);
}

void FixPropertyParticle::do_reverse_comm()
{
   //timer->stamp(); //FIXME: add the time spent
   if (commGhostRev)  comm->reverse_comm_fix(this);
   else error->all(FLERR,"FixPropertyParticle: Faulty implementation - reverse_comm invoked, but not registered");
...
double FixPropertyParticle::memory_usage()
{
    int nmax = atom->nmax;
...
void FixPropertyParticle::grow_arrays(int nmax)
{
    if (data_style) memory->grow(array_atom,nmax,nvalues,"FixPropertyParticle:array_atom");
    else memory->grow(vector_atom, nmax, "FixPropertyParticle:vector_atom");
...
void FixPropertyParticle::copy_arrays(int i, int j, int delflag)
{
...
void FixPropertyParticle::pre_set_arrays()
{
...
void FixPropertyParticle::set_arrays(int i)
{
...
void FixPropertyParticle::set_all(double value)
{
...
int FixPropertyParticle::pack_exchange(int i, double *buf)
{
...
int FixPropertyParticle::unpack_exchange(int nlocal, double *buf)
{
...
int FixPropertyParticle::pack_restart(int i, double *buf)
{
...
void FixPropertyParticle::unpack_restart(int nlocal, int nth)
{
...
int FixPropertyParticle::maxsize_restart()
{
  return nvalues+1;
}
...
int FixPropertyParticle::size_restart(int nlocal)
{
  return nvalues+1;
}
...
int FixPropertyParticle::pack_comm(int n, int *list, double *buf,
...
void FixPropertyParticle::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
int FixPropertyParticle::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void FixPropertyParticle::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m = 0;
  for (i = 0; i < n; i++) {
    j = list[i];
...
void FixPropertyParticle::write_restart(FILE *fp)
{
  int n = 0;
  double list[1];
...
void FixPropertyParticle::restart(char *buf)
{
  int n = 0;
...
double FixPropertyParticle::compute_vector(int n)
{
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
