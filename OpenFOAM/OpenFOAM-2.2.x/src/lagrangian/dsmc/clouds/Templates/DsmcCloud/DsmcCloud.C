...
template<class ParcelType>
void Foam::DsmcCloud<ParcelType>::setElectricField()
{
    // retrieve non-const access to electric field from the database
    volVectorField& Ef =
        const_cast<volVectorField&>
        (
           mesh_.lookupObject<volVectorField>("Ef")
        );
    ...
    Info << "Electric field was interpolated @xp " << endl;	
}
...
void Foam::DsmcCloud<ParcelType>::resetFields()
{
    ...
    rhoe_ = dimensionedScalar("zero",  dimensionSet(0, -3, 1, 0, 0, 1, 0), 0.0);
...
void Foam::DsmcCloud<ParcelType>::calculateFields()
{
    ...
    scalarField& rhoe = rhoe_.internalField();
    ...
    rhoe *= nParticle_/mesh().cellVolumes();
...
template<class ParcelType>
Foam::scalar Foam::DsmcCloud<ParcelType>::initCharge
(
    scalar charge
)
{
